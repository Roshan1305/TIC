...
app.post("/subscribe", (req, res) => {
  try {
    ...
    Subscribe.findOne(
      { subscribe_email: req.body.subscribe },
      (err, foundsubscribe) => {
        if (!err) {
          if (foundsubscribe) {
            ...
            var mailOptions = {
              ...
              html:
                "<div> You are Succesfully Suscribed to SASTRA IOTClub" +
                "</div>",
            };
    ...
    if (!err) {
      if (found.length === 0) {
        ...
        found.map((singleyear) => {
          if (
            singleyear.year == `${req.body.startyear} - ${req.body.endyear}`
          ) {
            count++;
          }
        });
  ...
  Admin.findOne({ email: req.body.email }, (err, found) => {
    if (!err && found) {
      bcrypt.compare(req.body.password, found.password, function (err, result) {
        if (!err && result) {
          const access = jwt.sign({ email: found.email }, "AccessGiven", {
            expiresIn: "300s",
          });
          const refresh = jwt.sign({ email: found.email }, "TokenIssued", {
            expiresIn: "7d",
          });
